...
def respond(sock):
    ...
    parts = request.split()
    #print(parts)
    #print(parts[1]) is the file?



    
    # Cat localhost (Not sure if needed but the lab video has it)
    if parts[1] == "/":
    ...
    # Access to docroot shown in get_options() function  
    docroot_files = options.DOCROOT + file
    #print(docroot_files)
    
    # Forbidden error
    if "~" in file or ".." in file:
        # First transmit is hidden?
        ...
        try:
            # Read the docroot files and transmit it
            with open(docroot_files, "r") as f:
                content = f.read()
                #print(content)
                #transmit(STATUS_OK, sock)
                transmit(STATUS_OK, sock)
        ...
        # If the file is not found in docroot_files 
        except FileNotFoundError:
            transmit(STATUS_NOT_FOUND, sock)
            transmit(STATUS_NOT_FOUND, sock)
            # Unable to transmit above the message
            transmit("\nFile DNE\n",sock)
